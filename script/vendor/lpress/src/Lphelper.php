...
class Lphelper
{
	...
	public static function ConfigCategory($type,$select = ''){
		...
		if (Auth::user()->role_id==3) {
			$categories = Category::where('user_id',Auth::id())->whereNull('p_id')->where('name','!=','Uncategorizied' )
//                ->select('id','name','p_id')
                ->where('type',$type)
			...
			$categories = Category::whereNull('p_id')->where('name','!=','Uncategorizied' )
//                ->select('id','name','p_id')
                ->where('type',$type)
		->with('childrenCategories')
		->get();
        ...
        if($type != 'category'){
            foreach ($categories as $cat) {
                $cat->name = json_decode($cat->name, true)[app()->getLocale()];
            }
        }
        //
