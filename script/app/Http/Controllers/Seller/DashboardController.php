...
//        dd(app()->getLocale());
	...
	public function staticData()
	{
		...
		$orders=Order::where('user_id',$user_id)->whereYear('created_at', '=',$year)->selectRaw('year(created_at) year, monthname(created_at) month, count(*) sales')
                ->groupBy('year', 'month')
                ->get();

        $earnings=Order::where('user_id',$user_id)->whereYear('created_at', '=',$year)->where('status','completed')->selectRaw('year(created_at) year, monthname(created_at) month, sum(total) total')
                ->groupBy('year', 'month')
                ->get();
	...
	public function perfomance($period)
	{
		$user_id=Auth::id();
		if ($period != 365) {
			$earnings=Order::where('user_id',$user_id)->whereDate('created_at', '>', Carbon::now()->subDays($period))->where('status','!=','canceled')->selectRaw('year(created_at) year, date(created_at) date, sum(total) total')->groupBy('year','date')->get();
		}
		else{
			$earnings=Order::where('user_id',$user_id)->whereDate('created_at', '>', Carbon::now()->subDays($period))->where('status','!=','canceled')->selectRaw('year(created_at) year, monthname(created_at) month, sum(total) total')->groupBy('year','month')->get();
		}
