...
class InventoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (!empty($request->src)) {
            $posts=Stock::where('sku','LIKE','%'.$request->src.'%')->whereHas('term')->with('term')->paginate(50);
        }
        elseif(!empty($request->status)){
            if ($request->status=='in') {
                $posts=Stock::where('stock_status',1)->whereHas('term')->with('term')->paginate(50);
            }
            else{
                $posts=Stock::where('stock_status',0)->whereHas('term')->with('term')->paginate(50);
            }


        }
        else{
           $posts=Stock::with('term')->whereHas('term')->paginate(30);
        }


       $src=$request->src ?? '';
       $status=$request->status ?? '';
       $in_stock=Stock::where('stock_status',1)->count();
       $out_stock=Stock::where('stock_status',0)->count();
       $total=Stock::count();
