                    ...
                    <form method="post" action="{{ route('admin.role.store') }}" class="basicform_with_reload">
                        ...
                        <div class="pt-20">
                            @php
                                ...
                                $arr['title']= 'اسم الدور عربي';
                                $arr['id']= 'name_ar';
                                $arr['type']= 'text';
                                $arr['placeholder']= 'اسم الدور عربي';
                                $arr['name']= 'name_ar';
                                $arr['is_required'] = true;
                                echo  input($arr);

                                $arr['title']= 'اسم الدور انجليزي';
                                $arr['id']= 'name_en';
                                $arr['type']= 'text';
                                $arr['placeholder']= 'اسم الدور انجليزي';
                                $arr['name']= 'name_en';
                                $arr['is_required'] = true;
                                echo  input($arr);

                                }else{
                                    $arr['title']= 'Role Name arabic';
                                $arr['id']= 'name';
                                $arr['type']= 'text';
                                ...
                                <div class="col-sm-12">
                                    <div class="custom-control custom-checkbox">
                                        ...
                                        <label class="custom-control-label checkAll"
                                               for="customCheck12">{{__('Permissions')}}</label>
                                    </div>
                                    <hr>
                                    @php $i = 1; @endphp
                                            ...
                                            <div class="col-3">
                                                <div class="form-check">
                                                    <input type="checkbox" class="form-check-input"
                                                           id="{{ $i }}Management" value="{{ $group->group_name }}"
                                                           onclick="checkPermissionByGroup('role-{{ $i }}-management-checkbox', this)">
                                                    <label class="form-check-label"
                                                           for="checkPermission">{{__($group->group_name)}}</label>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-9 role-{{ $i }}-management-checkbox">
                                                ...
                                                @foreach ($permissions as $permission)
                                                    <div class="form-check">
                                                        ...
                                                        <label class="form-check-label"
                                                               for="checkPermission{{ $permission->id }}">{{__($permission->name)}}</label>
                                                    </div>
                                                    @php  $j++; @endphp
                                                @endforeach
                                                <br>
