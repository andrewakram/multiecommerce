                    ...
                    <div class="card-header">
                        <div class="col-sm-10">
                            <ul class="nav nav-pills">
                                <li class="nav-item">
                                    <a class="nav-link @if($type==='all') active @endif"
                                       href="{{ route('admin.order.index') }}">{{ __('All') }}</a>
                                </li>

                                <li class="nav-item">
                                    <a href="{{ route('admin.order.index','status=2') }}"
                                       class="nav-link @if($type==2) active @endif">{{ __('Pending') }}</a>
                                </li>
                                <li class="nav-item">
                                    <a href="{{ route('admin.order.index','status=3') }}"
                                       class="nav-link @if($type==3) active @endif">{{ __('Expired') }}</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link @if($type===0) active @endif"
                                       href="{{ route('admin.order.index','status=cancelled') }}">{{ __('Cancelled') }}</a>
                                </li>
                        ...
                        <div class="col-sm-2">
                            @can('order.create')
                                <a href="{{ route('admin.order.create') }}"
                                   class="btn btn-primary float-right">{{ __('Create order') }}</a>
                            @endcan
                        </div>
                    </div>
                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-nowrap card-table text-center">
                                    ...
                                    @foreach($posts ?? [] as $key => $row)
                                        <tr>
                                            <td class="text-left"><input type="checkbox" name="ids[]"
                                                                         value="{{ $row->id }}"></td>
                                            <td class="text-left"><a
                                                    href="{{ route('admin.order.invoice',$row->id) }}">{{ $row->order_no }}</a>
                                            </td>
                                            <td>{{ $row->created_at->format('d-F-Y') }}</td>
                                            <td>
                                                <a href="{{ route('admin.customer.show',$row->user->id) }}">{{ $row->user->name }}</a>
                                            </td>
                                            <td>{{ amount_format($row->amount) }}</td>
                                            <td>
                                                @if(app()->getLocale() == "ar")
                                                    {{ $row->payment_method->method->name_ar ?? '' }}
                                                @else
                                                    {{ $row->payment_method->method->name_en ?? '' }}
                                                @endif
                                            </td>
                                            ...
                                            <td>
                                                @if($row->status == 1) <span
                                                    class="badge badge-success">{{__('Approved')}}</span> @elseif($row->status == 2)
                                                    <span
                                                        class="badge badge-warning">{{ __('Pending') }}</span>@elseif($row->status == 3)
                                                    <span class="badge badge-danger">{{ __('Expired') }}</span>@else
                                                    <span class="badge badge-danger">{{ __('Cancelled') }}</span> @endif
                                                ...
                                                <div class="dropdown d-inline">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item has-icon"
                                                           href="{{ route('admin.order.edit',$row->id) }}"><i
                                                                class="far fa-edit"></i> {{ __('Edit') }}</a>
                                                        <a class="dropdown-item has-icon"
                                                           href="{{ route('admin.order.show',$row->id) }}"><i
                                                                class="far fa-eye"></i> {{ __('View') }}</a>
                                                        <a class="dropdown-item has-icon"
                                                           href="{{ route('admin.order.invoice',$row->id) }}"><i
                                                                class="fa fa-file-invoice"></i> {{ __('Download Invoice') }}
                                                        </a>
