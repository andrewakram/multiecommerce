@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>trans('Edit Order')])
@endsection
@section('content')
<div class="row">
	<div class="col-12">
		<div class="card">
			...
			<div class="card-body">
				...
				<form class="basicform" action="{{ route('admin.order.update',$info->id) }}" method="post">
					...
					<small class="text-center">{{__('Note:')}} <span class="text-danger">{{ __('If This Order Does Not Used Have Any Payment Getway The Payment Status Will Not Update') }}</span></small>
				</form>
			</div>
		</div>
	</div>
