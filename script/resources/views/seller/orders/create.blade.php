@extends('layouts.app')
@section('head')
    @include('layouts.partials.headersection',['title'=> trans('Create Order')])
@endsection
@section('content')
    @php
        ...
        <div class="card-header">
            ...
            <form class="card-header-form">
                <div class="input-group">
                    <input type="number" required="" name="search" class="form-control"
                                        ...
                                        <select class="form-control" name="variation[]">
                                            ...
                                            @foreach($row->attributes as $attribute)
                                                <option value="{{ $attribute->id }}">
                                                    ...
                                                    {{--                                        @if(app()->getLocale() == "ar")--}}
                                                    {{--                                            {{ json_decode($attribute->attribute->name)->ar }} - {{ json_decode($attribute->variation->name)->ar }}--}}
                                                    {{--                                        @else--}}
                                                    {{--                                            {{ json_decode($attribute->attribute->name)->ar }} - {{ json_decode($attribute->variation->name)->en }}--}}
                                                    {{--                                        @endif--}}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        @if(count($row->options) > 0)
                                            <select class="form-control option" name="option[]">
                                                ...
                                                @foreach($row->options as $option)
                                                    <option @if($option->type == 1) disabled @endif>
                                                        @if(app()->getLocale() == "ar")
                                                            {{ $option->name }}
                                                        @else
                                                            {{ $option->name }}
                                    ...
                                    <td>
                                        <div class="input-group">
                                            <input type="number" class="form-control"
                                                   @if($row->stock->stock_manage == 1) max="{{ $row->stock->stock_qty }}"
                                                   min="0" @endif required="" value="1" name="qty">
                                            <div class="input-group-append">
                                                <button onclick="assignId({{ $row->id }})"
                                                        @if($row->stock->stock_status == 0) disabled=""
                                                        @endif class="btn btn-primary btn-icon"
                                                        id="submitbtn{{ $row->id }}" type="submit"><i
