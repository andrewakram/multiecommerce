@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>trans('Plans')])
@endsection
@section('content')
		...
		<table class="table table-hover table-nowrap card-table text-center">
			...
			<tbody class="list font-size-base rowlink" data-link="row">
				...
				<tr>
					...
					<td>
                        @if(!empty($row->payment_method->method->name))
                            @if(app()->getLocale() == "ar")
                                {{ json_decode($row->payment_method->method->name)->ar ?? '' }}
                            @else
                                {{ json_decode($row->payment_method->method->name)->en ?? '' }}
                            @endif
                        @else
					...
					<td>
						@if($row->status == 1) <span class="badge badge-success">{{__('Approved')}}</span> @elseif($row->status == 2) <span class="badge badge-warning">{{ __('Pending') }}</span>@elseif($row->status == 3) <span class="badge badge-danger">{{ __('Expired') }}</span>@else <span class="badge badge-danger">{{ __('Cancelled') }}</span> @endif
