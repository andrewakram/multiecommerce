@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>trans('Variations')])
@endsection
@section('content')
<div class="row">
        ...
        <form method="post" action="{{ route('seller.attributes-terms.destroy') }}" class="basicform_with_reload">
          ...
          <div class="table-responsive">
            <table class="table table-striped table-hover text-center table-borderless">
             ...
             <span class="text-danger text-center"><span class="text-dark">{{__('Note:')}}</span> {{ __('Before Delete Any Item Please Make Sure Product Count Must Be "0" Otherwise Product Will Not Append On Your Site') }} </span>
           </div>
         </form>
       </div>
     </div>
