@extends('layouts.app')
@section('head')
    @include('layouts.partials.headersection',['title'=>trans('Product Options')])
@endsection
@section('content')
                    ...
                    <div class="row">
                            ...
                            <ul class="nav nav-pills flex-column">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link active"
                                       href="{{ url('seller/product/'.$info->id.'/option') }}"><i
                        ...
                        <div class="col-sm-9">
                            <form class="basicform" method="post"
                                  action="{{ route('seller.product.option_update',$info->id) }}">
                                @csrf
                                <div class="row">

                                    <div class="col-12 col-md-12 col-lg-12">
                                        <button type="button" data-toggle="modal" data-target="#add_option"
                                                class="btn btn-primary float-right">{{ __('Add New Option') }}</button>
                                        ...
                                        <div id="accordion">
                                            @foreach ($info->options as $key=> $row)
                                                ...
                                                <div class="accordion option{{ $row->id }}">
                                                    <div class="accordion-header h-50" role="button"
                                                         data-toggle="collapse" data-target="#panel-body-{{ $key }}">
                                                        <div class="float-left">
                                                            <h6><span id="option_name{{ $row->id }}">
                                                        {{$row->name}}
                                                                    {{--                                                        @if(app()->getLocale() == "ar")--}}
                                                                    {{--                                                            {{ json_decode($row->name)->ar }}--}}
                                                                    {{--                                                        @else--}}
                                                                    {{--                                                            {{ json_decode($row->name)->en }}--}}
                                                                    {{--                                                        @endif--}}
                                                    </span> @if($row->is_required == 1) <span
                                                                    class="text-danger">*</span> @endif</h6>
                                                        </div>
                                                        <div class="float-right">
                                                            <a class="btn btn-success btn-sm text-white row_edit"
                                                            ...
                                                            <a class="btn btn-danger btn-sm text-white option_delete"
                                                            ...
                                                            <div class="option-values clearfix" id="option-2-values">
                                                                <div class="option-select ">
                                                                            ...
                                                                            @foreach ($row->childrenCategories ?? [] as $item)
                                                                                <tr class="option{{ $item->id }}">
                                                                                    ...
                                                                                    <td class="text-center">
                                                                                        <button type="button"
                                                                                                class="btn btn-white delete-row bg-white option_delete text-danger"
                                                                                                data-id="{{ $item->id }}">
                                                                    ...
                                                                    <button type="button" data-toggle="modal"
                                                                            data-target="#new_row_modal"
                                                                            class="btn btn-primary add_new_row"
                                                                            data-id="{{ $row->id }}">
                                                                        {{ __('Add New Row') }}
                                                                    </button>
                                                                </div>
...
{{--                                            <button type="submit"--}}
{{--                                                    class="btn btn-primary basicbtn">{{ __('Save') }}</button>--}}
{{--                                        @endif--}}
                                    </div>
    ...
    <div class="modal fade" id="add_option" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('seller.product.store_group',$info->id) }}" class="basicform_with_reload"
                  method="post">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="name">{{ __('Option Name') }} {{__('Arabic')}}</label>
                            <input type="text" name="name_ar" required class="form-control">
                        </div>
                        <div class="form-group">
                            <label for="name">{{ __('Option Name') }} {{__('English')}}</label>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('seller.product.add_row') }}" class="basicform_with_reload" method="post">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="add_row">{{ __('Row Name') }} {{__('Arabic')}}</label>
                            <input type="text" id="add_row_ar" class="form-control" name="name_ar" required>
                            <input type="hidden" id="row_id" name="row_id">
                        </div>
                        <div class="form-group">
                            <label for="add_row">{{ __('Row Name') }} {{__('English')}}</label>
                            <input type="text" id="add_row_en" class="form-control" name="name_en" required>
                        </div>
                        <div class="form-group">
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('seller.product.row_update') }}" class="basicform row_update_form">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="name">{{ __('Option Name') }}  {{__('Arabic')}}</label>
                            <input type="text" id="edit_name_ar" name="name_ar" required class="form-control">
                        </div>
                        <div class="form-group">
                            <label for="name">{{ __('Option Name') }}  {{__('English')}}</label>
                            <input type="text" id="edit_name_en" name="name_en" required class="form-control">
                        </div>
                        <div class="form-group">
