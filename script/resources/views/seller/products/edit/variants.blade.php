...
@endpush
@section('head')
@include('layouts.partials.headersection',['title'=>trans('Product Varient')])
@endsection
@section('content')
										...
										<tr class="attr_{{ $i }}">
											<td>
												<select  data-id="{{ $i }}" class="form-control parent_attr selec{{ $i }}">
													...
													<option data-parentattribute="{{ $row->childrenCategories }}" value="{{ $row->id }}"
                                                            @if($key == $row->id) selected @endif>{{ $row->name }}
                                                    </option>
													@endforeach
												</select>
											</td>
											<td >
												<select name="child[{{ $key }}][]" multiple  class="form-control select2 multislect child{{ $i }}" >
													...
													@if($key == $post->id)
														...
														<option class="attr{{ $i }}" value="{{ $item->id }}"
                                                                @if(in_array($item->id,$attribute)) selected @endif>{{ $item->name }}
                                                        </option>
														@endforeach
