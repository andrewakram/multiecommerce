@extends('layouts.app')
@section('head')
@include('layouts.partials.headersection',['title'=>trans('SEO')])
@endsection
@section('content')
				...
				<div class="row">
					...
					<div class="col-sm-9">
						...
						<div class="form-group">
                            ...
                            @if(app()->getLocale() == 'ar')
							    <input type="text" name="meta_title" class="form-control" required="" value="{{ $json['meta_title']['ar'] }}">
                            @else
                                <input type="text" name="meta_title" class="form-control" required="" value="{{ $json['meta_title']['en'] }}">
                            @endif
						</div>
						<div class="form-group">
							<label>{{ __('Meta Keyword') }}</label>
                            @if(app()->getLocale() == 'ar')
                                <input type="text" name="meta_keyword" class="form-control" required="" value="{{ $json['meta_keyword']['ar'] }}">
                            @else
                                <input type="text" name="meta_keyword" class="form-control" required="" value="{{ $json['meta_keyword']['en'] }}">
                            @endif
						...
						<div class="form-group">
                            ...
                            @if(app()->getLocale() == 'ar')
                                <textarea class="form-control" name="meta_description" required="" >{{ $json['meta_description']['ar'] }}</textarea>
                            @else
                                <textarea class="form-control" name="meta_description" required="" >{{ $json['meta_description']['en'] }}</textarea>
                            @endif
