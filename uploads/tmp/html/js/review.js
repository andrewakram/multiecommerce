$( document ).ready(function() {
    ...
    document.querySelectorAll('#star_review1').forEach(function(element) {
		element.addEventListener("mouseenter", function(event) {
			$('#stars_confirm1').html('');
			var stars = event.target.dataset.number;
			$('#star_input1').val(stars);
			var count = 0;
			document.querySelectorAll('#star_review1').forEach(function(el) {
				if (count < stars) {
					$(el).removeClass('fa-star-o');
					$(el).addClass('fa-star');
					$('#stars_confirm1').append('<i class="fa fa-star" aria-hidden="true" style="color:#212529 !important"></i>');
				} else {
					$(el).removeClass('fa-star');
					$(el).addClass('fa-star-o');
				}
